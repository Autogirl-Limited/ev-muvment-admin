...
- **NextAuth owns the browser session** using a JWT session cookie (`ev_next_auth`). The EV API access/refresh tokens are stored inside that encrypted, httpOnly token, so page JavaScript can't read them.
- **`proxy.ts`** guards routes optimistically from the NextAuth token, forces the change-password screen while the account is on a temporary password, and refreshes EV API tokens shortly before the access token expires. Refresh tokens are single-use, so refreshes are single-flight and the result is reused for 60s by requests still holding the old token.
...
- **`lib/auth/next-auth-options.ts`** defines the credentials provider used by the custom login form. `lib/auth/actions.ts` still holds non-login auth mutations: forgot/reset/change password and 2FA management. Plain data reads and writes go through TanStack Query instead (below).
- **Two-factor authentication is mandatory.** After sign-in, a user with no method is held on `/setup-2fa` (after any temporary-password change) until they enable one. The proxy gates on the user embedded in the NextAuth token, which is recomputed from the user the API returns on every login and token refresh; `requireUser()` re-checks the real account, so stale session data cannot let anyone in. The data gateway also refuses calls until it is done. The last remaining method cannot be disabled: the button is disabled in the UI and the Server Action refuses, since the API itself would allow it.
